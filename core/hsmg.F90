...
!----------------------------------------------------------------------
    ...
    call hsmg_tnsr(uc,mg_nh(l),uf,mg_nh(l+1),mg_jht(1,l),mg_jh(1,l))
    return
    end subroutine hsmg_rstr_no_dssum
!----------------------------------------------------------------------
!     computes
!     v = [A (x) A] u      or
!     v = [A (x) A (x) A] u
...
!     apply mask (zeros Dirichlet nodes)
!!!!! uncommenting
    call hsmg_do_wt(r,mg_mask(mg_mask_index(l,mg_fld)), &
    mg_nh(l),mg_nh(l),mg_nhz(l))
          
!     go to extended size array (room for overlap)
    if (if3d) then
        call hsmg_schwarz_toext3d(mg_work,r,mg_nh(l))
    else
        call hsmg_schwarz_toext2d(mg_work,r,mg_nh(l))
...
!    $             , ecrs2 (lx2*ly2*lz2*lelv)  ! quick work array
...
!     n = nx2*ny2*nz2*nelv
!     rmax = glmax(r,n)
!     if (nid.eq.0) write(6,*) istep,n,rmax,' rmax1'
...
!     if (param(41).eq.1) if_hybrid = .true.
    if_hybrid = .FALSE. 
    ...
    ! w := E e
        ...
        if (istep == 1) then
            ...
            if(nid == 0)write(6,1)istep,iter,rbd1dt,copt(1),copt1,'cpt1'
    ...
    !        rmax = glmax(mg_work2,nt)
    !        if (nid.eq.0) write(6,*) l,nt,rmax,' rmax2'
...
!            m-1
    ...
    ! ecrs := E e_c
        ...
        if(nid == 0)write(6,1)istep,iter,rbd1dt,copt(1),copt2,'cpt2'
    endif
! e := e + w
...
!  1.3540E+01  5.4390E+01  1.1440E+01  1.2199E+00  8.0590E+01 HSMG time

!  ==>  54/80 = 67 % of preconditioner time is in residual evaluation!
