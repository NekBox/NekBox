...
!-----------------------------------------------------------------------
    ...
    IF (IFFLOW) then
        ...
        IF (NDIM == 3) CALL COPY (SFZ,BFZ,NTOT1)
        CALL BDFORCE
    ENDIF
    ...
    !        Temperature field
        ...
        IF (HCODE(5,II) /= ' ') then
            ...
            DO 180 MEM=1,MEMTOT
                ...
                !                 This processor has a contribution
                    IEL = GLLEL(IEG)
                !                 call outmat(t,nx1,nx1,'t  out',mem)
                !                 call outmat(bq,nx1,nx1,'bq out',ifc)
                !                 call outmat(ts,nx1,nx1,'bq out',mem)
                    QINTEG(ISK,IOBJ)=QINTEG(ISK,IOBJ) - &
    ...
    !        One passive scalar field
    
        IF (HCODE(6,II) /= ' ' .AND. HCODE(6,II) /= '0') then
    ...
    IF (IFFLOW) then
        ...
        IF (NDIM == 3) CALL COPY (BFZ,SFZ,NTOT1)
    ENDIF

    return
    end subroutine intglq
!=======================================================================
    subroutine bdforce

!-----------------------------------------------------------------------

!     Compute total boundary force (components BFX,BFY,BFZ) on objects

!     Sign convention : these are the force exerted by
...
!     Compute total boundary flux-area product

!     Sign convention : flux enters the object from the continuum
        ...
        DO 150 MEM=1,MEMTOT
            ...
            !                 This processor has a contribution
                ...
                DO 200 IZ=KZ1,KZ2
                    DO 200 IY=KY1,KY2
                        DO 200 IX=KX1,KX2
                            SMULT(IX,IY,IZ,IEL)=SMULT(IX,IY,IZ,IEL) + 1.0
                200 END DO
            ENDIF
        150 END DO
    100 END DO
