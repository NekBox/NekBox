    ...
    include 'SIZE'
    if(nid == 0) write(6,*) 'ABORT: not compiled with CVODE support!'
    call exitt

    return
    end subroutine cv_setsize
!----------------------------------------------------------------------
    subroutine cv_init
...
! cvode will not allocate these arrays
    ...
    if (sizeOfLongInt() /= LongIntSize) then
        if(nid == 0) write(6,*) &
...
!     Note:  In contrast to the default CVODE version
!            cv_nglobal is defined as 'long long int'
...
! cvode will not allocate these arrays
    ...
    if(nid == 0) write(*,*) 'initializing ODE integrator ...'
...
!     Top level driver for the ODE integrator CVODE
!     webpage: https://computation.llnl.gov/casc/sundials

!     Integrate the IVP d/dt[y] = f(y(t),t); y(t=t0) := f0
!     using BDF(stiff) or AM(non-stiff).
!     f denotes the RHS function and is evaluated in fcvfun().
...
!     Compute Jacobian Vetor product FJV
    ...
    if(abs(PARAM(16)) == 3.1) ifcdjv = .TRUE. 
...
! set FJV = f(t, y + sigs*v/||v||)
    ...
    if(ifcdjv) then ! approximate Jacobian by 2nd-order fd quotient
        ...
        siginv = 1./(2*sig)
        do i = 1,NEQL
        !           FJV(i) = FJV(i)*siginv - FJV_(i)*siginv
...
!     extrapolated volocity at t=time_
...
! use TLAG as scratch space (this can get tricky!)
    ...
    if (time_ == timel) return

!      if(nid.eq.0) write(6,*) 'recompute extrapolated velocity'
    timel = time_
...
! update fine grid velocity
    if (param(99) > 0) call set_convect_new(vxd,vyd,vzd,vx,vy,vz)
...
!     Compute RHS function f (allocated within cvode)
!     NOTE: working array is ydot, do not change!
    ...
    if (cv_time /= timel) then
        timel = cv_time
        if(nid == 0) write(6,10) cv_time
...
! extrapolate velocity using CVODE's internal time
    if(abs(PARAM(16)) == 3) call update_vel(cv_time)
