...
!     Given an input vector v, this returns the additive Schwarz solution

!                       -1
!                  u = M   v

!                      T     -1
!     where M = sum ( R_i A_i    R_i )
!                i

!     The R_i's are simply index_set restriction operators.
...
!     Now solve each subdomain problem:

    etime1=dnekclock()

    eoff  = 0
    if (ifield > 1) eoff  = nelv
...
!     Exchange/add elemental solutions
    ...
    call dssum         (v1,nx1,ny1,nz1)
    call s_face_to_int (v1, 1.)
    if(param(42) == 0) call do_weight_op(v1)

!     Map back to pressure grid (extract interior values)
    ...
    return
    end subroutine local_solves_fdm
!-----------------------------------------------------------------------
...
!     l now holds the count matrix C on the outer pressure nodes
    if (if3d) then
        do e=1,nelv
            eb = e0+e
            do k=1,nz2
