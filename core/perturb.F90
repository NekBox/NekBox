...
!     Driver for perturbation velocity
    ...
    do jp=1,npert
        ...
        if (nid == 0 .AND. igeom == 2) write(6,1) istep,time,jp
...
!     Keep old Vp-field(s)
    ...
    return
    end subroutine lagfieldp
!-----------------------------------------------------------------------
    subroutine makefp

!     Make rhs for velocity perturbation equation
...
!     at current time step.
    ...
    return
    end subroutine advabp
!--------------------------------------------------------------------
    subroutine advabp_adjoint

!     Eulerian scheme, add convection term to forcing function
!     at current time step for backward part of adjoint:
!     Convective term is now (U.Grad)u - (Grad U)^T .u
!     instead of (u.Grad)U + (U.Grad)u in above subroutine advabp
...
!-----------------------------------------------------------------------
    ...
    return
    end subroutine makeabqp

    subroutine makebdqp
!-----------------------------------------------------------------------

!     Add contributions to Q from lagged BD terms.
...
! Most likely, the following can be commented out. (pff, 1/6/2010)
    if (npert > 1 .OR. ifbase)            ifprjp= .FALSE. 
...
!     Pressure extrapolation
    ...
    return
    end subroutine extrapprp
!-------------------------------------------------------------------
    subroutine lagpresp

!     save old pressure values
...
!-------------------------------------------------------------------
    ...
    !        Have to do lag terms as well, etc
    
    !        Also, must orthogonalize
    enddo

    if (nid == 0) then
        if (npert == 1) write(6,1) istep,time,(sigma(j),j=1,npert)
        if (npert == 2) write(6,2) istep,time,(sigma(j),j=1,npert)
        if (npert == 3) write(6,3) istep,time,(sigma(j),j=1,npert)
        if (npert == 4) write(6,4) istep,time,(sigma(j),j=1,npert)
        if (npert == 5) write(6,5) istep,time,(sigma(j),j=1,npert)
        if (npert == 6) write(6,6) istep,time,(sigma(j),j=1,npert)
        if (npert == 7) write(6,7) istep,time,(sigma(j),j=1,npert)
        if (npert == 8) write(6,8) istep,time,(sigma(j),j=1,npert)
        if (npert == 9) write(6,9) istep,time,(sigma(j),j=1,npert)
    endif
...
!-----------------------------------------------------------------------
    ...
    return
    end subroutine pert_ortho_norm
!-----------------------------------------------------------------------
    ...
    return
    end subroutine pert_ortho_norm1
!-----------------------------------------------------------------------
    ...
    if (opnorm2 > 0) opnorm2=sqrt(opnorm2/volvm1)
    return
    end function opnorm2
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
    ...
    !        For a  fresh simulation, then we wait 5 vertical diffusion
    !        times before we start measuring, so in this case just rescale
        ...
        if (jpp == 1) icount = icount + 1
    endif

    irescale = param(128)
    if (icount == irescale) then
        ...
        if(nid == 0) then        ! write out results to the .lyp file
        ...
        if_ortho_lyap = .FALSE. 
        if (param(125) > 0) if_ortho_lyap = .TRUE. 
        if (jpp == npert .AND. if_ortho_lyap) call pert_ortho_norm

    endif

    return
    end subroutine computelyap1
!-----------------------------------------------------------------------
    ...
    if (nid == 0) write(6,1) istep,pertnorm,pertinvnorm,jpp,'PNORM'
    1 format(i4,1p2e12.4,i4,a5)
    pertnorm = pertnorm*pertinvnorm

    return
    end subroutine rescalepert
!-----------------------------------------------------------------------
