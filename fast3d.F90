...
!     Generate fast diagonalization matrices for each element
    ...
    !        n12 = min(9,nr)
    !        write(6,1) e,'1D lr',llr(e),lmr(e),lrr(e),(lr(k),k=1,n12)
    !        write(6,1) e,'1D ls',lls(e),lms(e),lrs(e),(ls(k),k=1,n12)
    !        if (if3d)
    !    $   write(6,1) e,'1D lt',llt(e),lmt(e),lrt(e),(lt(k),k=1,n12)
    !   1    format(i6,1x,a5,1p12e12.4)
        ...
        if (if3d) then
            ...
            do k=1,nt
                do j=1,ns
                    do i=1,nr
                        ...
                        !                 write(6,3) e,'Reset Eig in gen fast:',i,j,k,l
            ...
            do j=1,ns
                do i=1,nr
                    ...
                    !                 write(6,2) e,'Reset Eig in gen fast:',i,j,l
                    !    $                         ,eps,diag,lr(i),ls(j)
...
!     In the present scheme, each element is viewed as a d-fold
!     tensor of (1+nz+1) arrays, even if funky bc's are applied
!     on either end of the 1D array.
...
!     Set up the 1D geometry for the tensor-product based overlapping Schwarz

!     Upon return:

!       dx() contains the spacing required to set up the stiffness matrix.


!     Input:

!       lbc (rbc) is 0 if the left (right) BC is Dirichlet, 1 if Neumann.

!       ll is the space between the left-most Gauss point of the middle
!          element and the right-most Gauss point of the LEFT element

!       lm is the space between the left-most Gauss point of the middle
!          element and the right-most Gauss point of the MIDDLE element

!       lr is the space between the right-most Gauss point of the middle
!          element and the left-most Gauss point of the RIGHT element

!       --- if ll (lr) is very small (0), it indicates that there is no
!           left (right) spacing, and that the left (right) BC is Neumann.


!       z() is the array of nz Gauss points on the interval ]-1,1[.

!     Boundary conditions:

!     bc = 0  -- std. Dirichlet bc applied 2 points away from interior
!     bc = 1  -- Dirichlet bc applied 1 point away from interior (outflow)
!     bc = 2  -- Neumann bc applied on interior point (W,v,V,SYM,...)
...
!        dx0       dx1   dx2    dx3   dx5    dx5        dx6

!    bl        |<--ll-->|<------lm------>|<---lr--->|           br
!     0--------x-----|--x---x--------x---x--|-------x-----------0

!       left elem.         middle elem.         right elem.
!                   -1                     +1


!    "bl" = (extrapolated) location of Gauss point NX2-1 in left elem.

!    "br" = (extrapolated) location of Gauss point 2 in right elem.

!    Overlapping Schwarz applied with homogeneous Dirichlet boundary
!    conditions at "bl" and "br", and with a single d.o.f. extending
!    in to each adjacent domain.
...
!     Right end
    ...
    return
    end subroutine set_up_1D_geom
!-----------------------------------------------------------------------
    subroutine gen_eigs_A_fem(sf,sft,atd,n,l,lbc,rbc)

!     Set up Fast diagonalization solver for FEM on mesh 2
...
!     Fill initial arrays, A & B:
    ...
    !        il,ir are the left and right endpts of element ie.
        il = ie
        ir = ie+1
    
        if (ie > 0) ad(il) = ad(il) +       li(ie)
        if (ie < n) ad(ir) = ad(ir) +       li(ie)
        if (ie > 0) au(il) =        -       li(ie)
        if (ie > 0) bh(il) = bh(il) + 0.5 * l(ie)
        if (ie < n) bh(ir) = bh(ir) + 0.5 * l(ie)
    enddo

!     Take care of bc's (using blasting)
    ...
    enddo
!                                        ~
!     Scale rows and columns of A by C:  A = CAC

    do i=1,n
        atd(i) = c(i)*ad(i)*c(i)
    enddo

!     Scale upper diagonal
...
!     Make "like" eigenvectors of same sign (for ease of diagnostics)

    do j=1,n
        avg = vlsum(sf(1,j),n)
        if (avg < 0) call chsign(sf(1,j),n)
    enddo

!     Clean up zero eigenvalue

    eps = 1.0e-6*dmax
    do i=1,n
    !        if (atd(i).lt.eps)
    !    $      write(6,*) 'Reset Eig in gen_Afem:',i,n,atd(i)
        if (atd(i) < eps) atd(i) = 0.0
    enddo

!     scale eigenvectors by C:
    ...
    enddo
!                                        ^
!     Orthonormalize eigenvectors w.r.t. B inner-product
...
!     do j=1,n
!        do i=1,n
!           sft(i,j) = vlsc3(bh,sf(1,i),sf(1,j),n)
!        enddo
!     enddo

!     n8 = min(n,8)
!     do i=1,n
!        write(6,2) (sft(i,j),j=1,n8)
!     enddo
!   2 format('Id:',1p8e12.4)
...
!     Right end
    ...
    return
    end subroutine set_up_1D_geom_ax
!-----------------------------------------------------------------------
...
!     Clean up zero eigenvalue
    ...
    do i=1,n
    !        if (atd(i).lt.eps)
    !    $      write(6,*) 'Reset Eig in gen_Afm_ax:',i,n,atd(i)
        if (atd(i) < eps) atd(i) = 0.0
    enddo
...
! cs on E are from normal vel component
    if(lbc == 2 .OR. lbc == 3) then !wall,sym - dirichlet velocity
        eb0=1
    else !outflow,element - neumann velocity
        eb0=0
    endif
    if(rbc == 2 .OR. rbc == 3) then !wall,sym - dirichlet velocity
        eb1=n-1
    else !outflow,element - neumann velocity
        eb1=n
    endif
! cs on B are from tangent vel component
    if(lbc == 2) then !wall - dirichlet velocity
        bb0=1
    else !outflow,element,sym - neumann velocity
        bb0=0
    endif
    if(rbc == 2) then !wall - dirichlet velocity
        bb1=n-1
    else !outflow,element,sym - neumann velocity
        bb1=n
    endif
...
!     sets Neumann BC on pressure (ibc=2) for face and e(lement) except
!     when ifield normal component has (homogeneous) Neumann
!     boundary condition setting ibc=1 (i.e. Direchlet BC on pressure)
...
!     Note: 'SYM' on a plane with r,s,t-normal is 'dnn','ndn','nnd'? bsym?
    ...
    if (indx1(cbc(ied,e,ifield),'d',1) > 0)   ibc=2

    if (indx1(cbc(ied,e,ifield),'n',1) > nfc) ibc=1 ! 'n' for V_n
