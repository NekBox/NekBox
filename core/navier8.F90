...
!     Required storage for a:
    ...
    ! later. call map_m_to_n(hl1,nxc,h1(1,1,1,ie),nx1,if3d,w,ldw)
    ! later. call map_m_to_n(hl2,nxc,h2(1,1,1,ie),nx1,if3d,w,ldw)
...
!     gridpoints (e.g., an array defined by SEM-GLL vertices)

!         For example, suppose ndim=3.

!         Then, there would be ncrs_loc := nxc^3 dofs for this matrix,

!         and the matrix size would be (ncrs_loc x ncrs_loc).
...
!     Here, we march across sub-cubes
    ...
    do kz=1,nzc-1
        do ky=1,nyc-1
            ...
            !        call outmat(a_loc,ldm2,ldm2,'A_loc ',ie)
...
!     OUTPUT:    r(k) = rank of each entry,  k=1,..,n
!                maxr = max( r )
!                w(i) = sorted & compressed list of input values
    ...
    rlast = r(1)
    do i=1,n
    !        Bump rank only when r_i changes
        if (r(i) /= rlast) then
            rlast = r(i)
            maxr  = maxr + 1
        endif
...
!     H1 Iterpolation operator:  linear --> spectral GLL mesh
    ...
    return
    end subroutine map_c_to_f_l2_bilin
...
!                                 (linear --> spectral GLL mesh)
    ...
    return
    end subroutine map_f_to_c_l2_bilin
...
!               nn  = max(rank)
!               a(j,i) is destroyed
!               a(1,i) tally of preceding structure values
...
!     Find unique a's
    ...
    do i=2,n
        ...
        if (a_ne_b) then              ! new structure
            ...
            if (aa(2) == 0) ms = aa(2) ! structure type
...
!     Set a1() to number of preceding dofs
...
!     H1 Iterpolation operator:  linear --> spectral GLL mesh
    ...
    return
    end subroutine map_c_to_f_h1_bilin
...
!                                 (linear --> spectral GLL mesh)
    ...
    return
    end subroutine map_f_to_c_h1_bilin
!-----------------------------------------------------------------------
...
!     Sort edges by bounding vertices.
    do i=0,4*nel-1
        if (edge(0,i,1,1) > edge(1,i,1,1)) then
            kswap = edge(0,i,1,1)
