...
!-------------------------------------------------------------------------
    ...
    !        Uzawa decoupling: First, compute pressure.....
        ...
        IF (ICG > 0) CALL ADD2 (PR,WP,NTOT2)
...
!     Compute inhomogeneities for the elliptic solver in the pressure
!     residual evaluation.
!     INTYPE =  0  steady state
!     INTYPE =  1  explicit, Euler forward
...
!-----------------------------------------------------------------------
    ...
    !        Steady state or implicit scheme
...
!     Arrays in parameter list
    ...
    IF (PARAM(86) == 0.0) THEN
    !        Always use the convective form !!! (ER)
        CALL CONV1 (CONV,FI)
    ELSE
    !        Use skew-symmetric form
    
    !        Generate operator mask arrays CMASK1 and CMASK2
...
!     Generate maskarrays for the convection operator

!       CMASK1   =  1.0 in the interior
!                =  0.0 at outflow
!       CMASK2   =  0.0 in the interior
!                =  1.0 at outflow
...
!     Compute driving force (in x- and y-direction)
...
!     Convection of all velocity components.
!     Runge-Kutta scheme.
...
!     Operation count
    ...
    DO 1000 JLAG=ILAG,1,-1
        ...
        !           Sum up contributions from 4 stages.
        ...
        DO 100 IE=1,NELV
            DO 100 IFACE=1,NFACES
                ...
                IF (CB(1:1) == 'V' .OR. CB(1:1) == 'v') THEN
                    ...
                    IF (VAVER < 0.) CALL FACEV (HV1MSK,IE,IFACE,0.0,NX1,NY1,NZ1)
                ENDIF
...
!     Collocate the weights on mesh M2 with the pressure or the
...
!-----------------------------------------------------------------
    ...
    100 END DO
    return
    end subroutine genwp

    subroutine convuz (ifstuz)
!--------------------------------------------------------------------

!     Check convergence for the coupled form.
...
!     Momentum
    ...
    IF (RV1 > TCRITV) IFSTUZ = .FALSE. 
    IF (RV2 > TCRITV) IFSTUZ = .FALSE. 
    IF (NDIM == 3) THEN
        RV3 = SQRT(GLSC3(TA3,RESV3,VMULT,NTOT1)/VOLVM1)
        IF (RV3 > TCRITV) IFSTUZ = .FALSE. 
    ENDIF
...
!     begin block with ...exits to 40
    ...
    30 END DO
    info = 0
!     ......store inverse of the diagonal for repeated forward/back solves (pff)
    do 35 j=1,n
...
!     Find scale factors to normalize the first N-1 Legendre polynomials
!     such that  (L_i,L_j) = delta_ij
...
!     Compute D*FI (part of the convection operator)
...
!     interpolate ta1 and vx onto larger mesh
...
!     interpolate ta1 and vy onto larger mesh
...
!     Now, *project* DFID onto mesh 1 using L2 projection
        ...
        DO 100 IE=1,NELV
            DO 100 IFACE=1,NFACES
                ...
                IF (CB(1:1) == 'V' .OR. CB(1:1) == 'v') THEN
                    ...
                    IF (VAVER < 0.) CALL FACEV (mask,IE,IFACE,0.0,NX1,NY1,NZ1)
                ENDIF
                IF (CB(1:2) == 'WS' .OR. CB(1:2) == 'ws') &
...
!     DEL*[mue*(DEL V + (DEL V)^T - 2/3*DEL*V *I)]
!                 =
!     2*DEL*[mue*(S - 1/3*QTL*I)]

!     where mue is the viscosity, S the strain rate tensor,
!     tr(S) the trace of S and I the indentitiy matrix.

!     NOTE: for now only for incompressible flows
!           In the compressible case we need to compute S using
!           a kth-order extrapolation scheme because we cannot
!           use the existing MAKEABF extrapolater and we need
!           to use mue/QTL from the thermo-chemical subsystem.
!           CAUTION: we cannot use BFX,BFY,BFZ anymore. Some
!                    extra handling in plan4 is needed to add the
...
!-----------------------------------------------------------------------
    ...
    if (nid == 0 .AND. icalld == 0) write(6,*) 'nu_star:',nu_star
    icalld=1
    ...
    return
    end subroutine expl_strs_e
!-----------------------------------------------------------------------
    subroutine expl_strs_e_3d(w1,w2,w3,u1,u2,u3,e)

!     Evaluate, for element e,

!     /dvi\T  /     dui       duj \
!     |---|   | dnu ---  + nu --- |    (no boundary terms at present)
!     \dxj/   \     dxj       dxi /
...
!     Pointers into GLL-based derivative / interpolation operators
    ...
    if (ip == 0) then
        ...
        if (nid == 985) write(6,*) nstore,ldg,ij,md,mx,ip,' NSTOR'
...
!        dgt  = transpose of derivative matrix
