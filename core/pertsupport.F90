! Various support routines for perturbation related calculuations.
!------------------------------------------------------------------------
    subroutine flushBuffer(k)
    integer :: k

!     call flush(k)  ! this works most places (including the pgi compiler)
...
!------------------------------------------------------------------------
    ...
    if (opnorm > 0) opnormOld=sqrt(opnorm)
    return
    end function opnormOld
!-----------------------------------------------------------------------
...
!----------------------------------------------------------------------
    ...
    if(param(31) == 0) return !param(31) is number of exponents
    ...
    if(param(64) /= 1) then !fresh start, param(64) is restart flag
        ...
        if(nid == 0) write(6,*)jpp,'norm of pert IC=',pertnorm
        if_restart  = .TRUE. 
        pertInvNorm = 1.0/pertNorm
        call rescalepert(pertnorm,pertinvnorm,jpp)
        if(nid == 0) write(6,*) jpp,'Rescaled the perturbation'
        call rzero(lyap(1,jpp),3)
        lyap(3,jpp) = pertnorm
    endif
...
!     opening the lyp file in unit=79
    ...
    if(if_restart) then
        ...
        if(timeStart > time) then
            write(6,*) nid,' reporting! timestart > time! time=',time
            call exitt
        endif
    endif
