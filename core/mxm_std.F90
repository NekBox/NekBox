...
!-----------------------------------------------------------------------
    ...
    if (icalld == 0) then    !     Initialize matrices:
        icalld = icalld + 1
        time1 = dnekclock()
        call initab(a,b,lt)
        time2 = dnekclock()-time1
        if (nid == 0) write(6,*) 'mxm test init:',lt,time2,name
    endif
...
!     Rectangular matrix tests
    ...
    do n=nn0,nn1
        ...
        if (lt > n1*n3) then
            ...
            if (name == 'madd ') loop = 200000/(n1*n3) + 5000
        ...
        !-------------------------------------------------------
            ...
            if (name == 'madd ') iops = loop*n1*n3
        !         write(6,*) loop,time,time2,time1,overh
            flops=iops/(1.0e6*time)
            s(n,1) = flops
        
            timel = time/loop
            if (nid == 0) write(6,199) n,n1,n2,n3,flops,timel,name
        ...
        !-------------------------------------------------------
            ...
            if (nid == 0) write(6,198) n,n1,n2,n3,flops,timel,name
...
!-----------------------------------------------------------------------
    ...
    real ::        a(nn,2,nt,3)
!                   ^  ^ ^  |__ N^2xN, NxN, NxN^2
!  matrix order N __|  | |__________which mxm
!                      |
!                      |__cached vs. noncached data
    ...
    do n = nn0,nn1
        ...
        do it=1,nt
            ...
            di = 0.
            do k=1,3
                if (s(n,1,it,k) > 0) then  ! Take harmonic means of
            ...
            if (ai > 0) ai = di/ai
            a(n,1,it,1) = di/ai
            if (ai > fmax .AND. c(2,it) /= 'madd ') then
                fmax     = ai
                itmax(n) = it
            endif
        enddo
        it = itmax(n)
        if (nid == 0) write(6,3) n,it,c(2,it),(s(n,1,it,k),k=1,3),fmax
...
!     Case by case
    ...
    do k=1,3
        do n = nn0,nn1
            ...
            enddo
        enddo
        if (k == 1) call out_anal(s,a,nn,c,nt,itmax,'Case N2N',k,ivb)
        if (k == 2) call out_anal(s,a,nn,c,nt,itmax,'Case NxN',k,ivb)
        if (k == 3) call out_anal(s,a,nn,c,nt,itmax,'Case NN2',k,ivb)
    enddo
