...
!     ifcrsl = .true.
    ...
    if (matmod < 0) tol = 1.e-4
!     if (matmod.lt.0) tol = tin
!     tol = 1.e-10

    if (ifcrsl) call set_up_h1_crs_strs(h1,h2,ifield,matmod)

    if (nid == 0 .AND. istep == 1) write(6,6) matmod,tol,tin,param(22)
    ...
    if (rbnorm < tol**2) then
    !        if ( .not.ifprint )  goto 9999
        if (matmod >= 0 .AND. nid == 0) write (6,2000) istep,rbnorm,tol
        if (matmod < 0 .AND. nid == 0) write (6,2010) istep,rbnorm,tol
        goto 9999
    endif
...
!     call copy (dpc,binv,n)
    ...
    do 1000 iter=1,maxit
        ...
        if (rbnorm < tol) then
            ifin = iter
            if (nid == 0) then
                if (matmod >= 0) write(6,3000) istep,ifin,rbnorm,tol,r0
                if (matmod < 0) write(6,3010) istep,ifin,rbnorm,tol,r0
            endif
            goto 9999
        endif
    ...
    if (matmod >= 0 .AND. nid == 0) write (6,3001) istep,iter,rbnorm,tol
    if (matmod < 0 .AND. nid == 0) write (6,3011) istep,iter,rbnorm,tol
...
!     Check that the tolerances are not too small for the MG-solver.
!     Important when calling the MG-solver (Gauss-Lobatto mesh).
...
!     Single or double precision???
    ...
    Y     = 1.
    DIFF  = ABS(X-Y)
    IF (DIFF == 0.) EPS = 1.E-6*EIGGA/EIGAA
    IF (DIFF > 0.) EPS = 1.E-13*EIGGA/EIGAA
...
!     Kludge : for debugging purpose Fudge Factor comes in
!              as PARAM(45)
    ...
    IF (DTFS > 1.E+10) DTFS = 0.0
    DTFS = DTFS * FACTOR

    IF (DTFS == 0.0) THEN
        IF (ISTEP == 1 .AND. NID == 0) THEN
...
!     Compute solution to coupled Helmholtz equations
!     (stress formulation)
...
!     call set_up_h1_crs_strs(h1,h2,ifield,matmod)
    ...
    if (imesh == 1) then
        ...
        if (matmod < 0) then
            napprox(1) = 0
            iproj      = param(94)
            if (iproj > 0 .AND. istep > iproj) napprox(1)=param(93)
            napprox(1)=min(napprox(1),istep/3)
            call strs_project_a(r1,r2,r3,h1,h2,rmult,ifield,ierr,matmod)

        !         call opcopy(y(1,1),y(1,2),y(1,3),x(1),x(1+n),x(1+2*n))
    ...
    !        if (matmod.lt.0.and.istep.ge.11)
    !    $    call opcopy(v(1,1),v(1,2),v(1,3),u1,u2,u3)

        if (matmod < 0) &
        call strs_project_b(u1,u2,u3,h1,h2,rmult,ifield,ierr)

    !        if (matmod.lt.0.and.istep.ge.11) then
    !         ifxyo = .true.
    !         call outpost(y(1,1),y(1,2),y(1,3),pr,t,'   ')
    !         call outpost(v(1,1),v(1,2),v(1,3),pr,t,'   ')
    !         call outpost(u1    ,u2    ,u3    ,pr,t,'   ')
    !         call exitti('quit$',matmod)
    !        endif
...
!     Calculate coupled  Aij Uj  products
...
!     Add other Helmholtz contributions

    IF (IFH2 .AND. MATMOD >= 0) THEN
        CALL ADDCOL4 (AU1,BM1,H2,U1,NTOT1)
        CALL ADDCOL4 (AU2,BM1,H2,U2,NTOT1)
        IF (NDIM == 3) CALL ADDCOL4 (AU3,BM1,H2,U3,NTOT1)
    endif
...
!     x is an (NX,NY,NZ) data structure
...
!     Set up counters ;  fd is the dssum notation.
    call dsset(nx1,ny1,nz1)
    fd     = eface1(f)
    js1    = skpdat(1,fd)
    jf1    = skpdat(2,fd)
...
!     Pack vertex ids component-wise (u,v,w_1; u,v,w_2; etc.)
...
!     real    a(ncl,ndim,ncl,ndim,1),h1(1),h2(1)
    ...
    do k = 1,ndim
        ...
        do j = 1,ncl
            ...
            do e = 1,nel
                if (k == 1) call copy(w1(1,e),b(1,j),nxyz)
                if (k == 2) call copy(w2(1,e),b(1,j),nxyz)
                if (k == 3) call copy(w3(1,e),b(1,j),nxyz)
            enddo
    ...
    return
    end subroutine get_local_crs_galerkin_strs
!-----------------------------------------------------------------------
...
!        vc1(j)=1
!        call crs_solve(xxth_strs,uc1,vc1)
!        write(55,55) (uc1(i),i=1,mm)
!  55    format(1p1e22.13)
!     enddo
!     write(6,*) 'QUIT after io 55 write'
!     stop
...
!     Reorthogonalize basis
    ...
    if (nid == 0) write(6,6) istep,k,ierr,l2a,l2b
    6 format(i9,2i3,1p2e12.4,' h3proj')

    return
    end subroutine strs_project_a
!-----------------------------------------------------------------------
...
!     if (936.le.istep.and.istep.le.948) then
!     if (16.le.istep.and.istep.le.18) then
!        call outpost(x1,x2,x3,pr,vdiff,'tst')
!        call outpost(x(1),x(1+3*n),x(1+2*n),pr,t,'tst')
!     endif
    ...
    !        k2 = 1 + 1*n + (k-1)*m + m
    !        k3 = 1 + 2*n + (k-1)*m + m
    !        call opcopy(x(k1),x(k2),x(k3),xs(1),xs(1+n),xs(1+2*n))
    else
        k=k+1
        k1 = 1 + 0*n + (k-1)*m + m
        k2 = 1 + 1*n + (k-1)*m + m
...
!     if (k.eq.kmax) call opcopy(xs(1),xs(1+n),xs(1+2*n),x1,x2,x3) ! presave

    napprox(2)=k

    return
    end subroutine strs_project_b
!-----------------------------------------------------------------------
...
!     Orthonormalize the kth element of X against x_j, j < k.
    ...
    eps  = 1.e-8
    ierr = 0
    if (scale/xax0 < eps) ierr=1

    if(nid == 0 .AND. (istep < 10 .OR. mod(istep,100) == 0 .OR. ierr > 0)) &
...
!-----------------------------------------------------------------------
    ...
    js=1
    do j=1,k
        if (js < j) call copy(x(1,1,js),x(1,1,j),m)
